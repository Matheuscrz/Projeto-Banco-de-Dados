...
CREATE TYPE IF NOT EXISTS Assinatura_Status AS ENUM ('Ativo', 'Cancelado');
...
CREATE TYPE IF NOT EXISTS idioma_type AS ENUM ('Português', 'Inglês', 'Espanhol', 'Francês', 'Outro');
CREATE TYPE IF NOT EXISTS legenda_type AS ENUM ('Português', 'Inglês', 'Espanhol', 'Francês', 'Outro');

-- Table stream."Perfil"
CREATE TABLE IF NOT EXISTS stream.perfil (
  "Id" UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
  "Nome" VARCHAR(45) NOT NULL,
  "Idioma" idioma_type ARRAY NOT NULL DEFAULT ARRAY['Português'],
  "Legenda" legenda_type ARRAY DEFAULT ARRAY['Português'],
...
CREATE TYPE IF NOT EXISTS Visualizacao_Status AS ENUM ('Assistido', 'Incompleto');
...
CREATE TYPE IF NOT EXISTS Conteudo_Genero AS ENUM ('Terror', 'Ação', 'Romance');
CREATE TYPE IF NOT EXISTS classificacao AS ENUM ('Livre', '10', '12', '14', '16', '18');
...
CREATE TYPE IF NOT EXISTS Status_Temporada AS ENUM ('Lançamento', 'Em Breve');
...
CREATE TYPE IF NOT EXISTS Idioma_Legenda AS ENUM ('Português', 'Inglês', 'Espanhol', 'Francês', 'Outro');
...
CREATE TYPE IF NOT EXISTS modalidade AS ENUM ('Credito', 'Debito');
